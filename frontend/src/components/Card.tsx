...
const CardFooter = ({ machineData }: CardProps) => {
    const [machineError, setMachineError] = useState<ErrorlogResponse>();
    useEffect(() => {
        void getOneMachine(machineData.serialnumber).then((machineError) =>
    ...
    return (
        <div>
            {machineData.offline ? (
                ...
                <div>
                    {machineError ? (
                        <div className="w-full h-7 bg-red-500"></div>
                    ) : (
